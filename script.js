...
document.addEventListener("DOMContentLoaded", () => {
  ...
  class Particle {
    constructor(x, y, color) {
      this.x = x;
      this.y = y;
      this.radius = Math.random() * 5 + 2; // Larger particles
      this.color = color;
      this.opacity = 1;
      this.angle = Math.random() * Math.PI * 2;
      this.speed = Math.random() * 5 + 3; // Faster particles
      this.gravity = 0.03; // Slightly more gravity
      this.decay = 0.015; // Slower decay for lasting effect
    }
    ...
    update() {
      ...
      this.opacity -= this.decay; // Fade out
    }
  ...
  function createFirework(x, y) {
    ...
    // Create a larger number of particles for larger fireworks
    for (let i = 0; i < 100; i++) {
  ...
  function triggerFireworks() {
    const interval = setInterval(() => {
      ...
      const y = Math.random() * canvas.height / 2; // Top half of the screen
      createFirework(x, y);
    }, 400); // Fireworks interval

    setTimeout(() => {
      clearInterval(interval);
      setTimeout(() => {
        canvas.style.display = "none";
      }, 3000); // Allow remaining fireworks to fade out
    }, 7000); // Fireworks duration
  }

  // Trigger fireworks after the loading screen disappears
  const loadingScreen = document.getElementById("loading-screen");
  setTimeout(() => {
